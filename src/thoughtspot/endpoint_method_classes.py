...
class SharedEndpointMethods:
    ...
    def list(
        ...
        if self.metadata_subtype is None:
            return self.rest.metadata_listobjectheaders(
                object_type=self.metadata_name,
                sort=sort,
    ...
    def details(self, guid: str) -> Dict:
        ...
        if self.last_details is not None and self.last_details["storables"][0]["header"]["id"] == guid:
            ...
            details = self.rest.metadata_details(
                object_type=self.metadata_name,
                object_guids=[
                    guid,
                ],
    ...
    def assign_tags(self, object_guids: List[str], tag_guids: List[str]):
        ...
        response = self.rest.metadata_assigntag(
            object_guids=object_guids, object_type=obj_type_list, tag_guids=tag_guids
        )
        return response

    def share(self, object_guids: List, permissions: Dict):
        response = self.rest.security_share(
            shared_object_type=self.metadata_name, shared_object_guids=object_guids, permissions=permissions
        )
        return response

    def create_share_permissions(
        self,
        read_only_users_or_groups_guids: Optional[List[str]] = (),
        edit_access_users_or_groups_guids: Optional[List[str]] = (),
        full_access_users_or_groups_guids: Optional[List[str]] = (),
        remove_access_users_or_groups_groups: Optional[List[str]] = (),
    ) -> Dict:
    ...
    def transfer_object_ownership(self, object_guids: List[str], current_owner_username: str, new_owner_username: str):
        if object_guids is None:
            raise Exception()
        self.rest.user_transfer_ownership(
...
class UserMethods(SharedEndpointMethods):
    ...
    def list_available_objects_for_user(
        self, user_guid: str, minimum_access_level: str = "READ_ONLY", filter: Optional[str] = None
    ):
        return self.rest.metadata_listas(
            user_or_group_guid=user_guid,
    ...
    def transfer_ownership_of_all_objects_between_users(self, current_owner_username, new_owner_username):
        return self.rest.user_transfer_ownership(
            current_owner_username=current_owner_username, new_owner_username=new_owner_username
        )

    @staticmethod
    def get_user_element_for_user_sync(
        username: str,
        display_name: str,
...
class GroupMethods(SharedEndpointMethods):
    ...
    def list_available_objects_for_group(
        self, group_guid: str, minimum_access_level: str = "READ_ONLY", filter: Optional[str] = None
    ):
        return self.rest.metadata_listas(
            user_or_group_guid=group_guid,
    ...
    def remove_privilege_from_group(self, privilege: str, group_name: str):
        return self.rest.group_removeprivilege(
            privilege=privilege,
            group_names=[
                group_name,
            ],
    ...
    @staticmethod
    def get_group_element_for_user_sync(
        group_name: str,
        display_name: str,
...
class PinboardMethods(SharedEndpointMethods):
    ...
    def list_spotiqs(
        ...
        full_listing = self.rest.metadata_listobjectheaders(
            object_type=MetadataNames.PINBOARD, sort=sort, sort_ascending=sort_ascending, filter=filter
        )
        final_list = []
    ...
    def share_pinboards(
        self,
        shared_pinboard_guids: List[str],
        permissions: Dict,
        notify_users: Optional[bool] = False,
        message: Optional[str] = None,
        email_shares: Optional[List[str]] = None,
        use_custom_embed_urls: bool = False,
    ):
        self.rest.security_share(
            shared_object_type=MetadataNames.PINBOARD,
    ...
    def pdf_export(
        ...
        return self.rest.export_pinboard_pdf(
    ...
    def get_referenced_data_sources(self, guid):
        ...
        for ro in details["storables"][0]["header"]["resolvedObjects"]:
            tables = details["storables"][0]["header"]["resolvedObjects"][ro]["reportContent"]["sheets"][0][
                "sheetContent"
            ]["visualizations"][0]["vizContent"][
                "columns"
            ]  # ['column'] ## ['referencedTableHeaders']
...
class LiveboardMethods(PinboardMethods):
    ...
    def share_liveboards(
        self,
        shared_liveboard_guids: List[str],
        permissions: Dict,
        notify_users: Optional[bool] = False,
        message: Optional[str] = None,
        email_shares: Optional[List[str]] = None,
        ...
        self.rest.security_share(
            shared_object_type=MetadataNames.LIVEBOARD,
...
class AnswerMethods(SharedEndpointMethods):
    ...
    def share_answers(
        ...
        self.rest.security_share(
            shared_object_type=MetadataNames.ANSWER,
...
class TMLMethods:
    ...
    def import_tml(self, tml: Dict, create_new_on_server=False, validate_only=False, formattype="JSON"):
        return self.rest.metadata_tml_import(
            tml=tml, create_new_on_server=create_new_on_server, validate_only=validate_only, formattype=formattype
        )
    ...
    def upload_tml(self, tml, create_new_on_server=False, validate_only=False, formattype="JSON"):
        return self.import_tml(
            tml=tml, create_new_on_server=create_new_on_server, validate_only=validate_only, formattype=formattype
        )

    def publish_new(self, tml, formattype="JSON"):
        return self.import_tml(tml=tml, create_new_on_server=True, validate_only=False, formattype=formattype)
